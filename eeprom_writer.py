...
def read(address1, address2=None):
    if (address2 == None):
        address2 = address1 + 1
    else:
        address2 += 1
    ...
    for address in range(address1, address2):
        ...
        out.append(ord(ser.read(size = 1)))
        result = ord(ser.read(size = 1))
        if (result != Protocol.END.value):
            print hex(result)
            raise ProtocolException('Unexpected protocol message while awaiting response.', end)
        ...
        if uinput != '':
            ...
            if uinput[0] == 'quit' or uinput[0] == 'q' or uinput[0] == 'exit':
                ...
                if len(uinput) == 2:
                    a1 = int(uinput[1], 0)
                    out = read(a1)
                    print '[' + hex(a1) + '] = ' + hex(out[0])
                elif len(uinput) == 3:
                    a1 = int(uinput[1], 0)
                    a2 = int(uinput[2], 0)
                    if a2 <= a1:
                        print 'Second address must be greater than the first.'
                        continue
                    
                    out = read(a1, a2)
                    for i in range(0, a2 - a1 + 1):
                        print '[' + hex(a1 + i) + '] = ' + hex(out[i])
                else:
